...
class TaskResultRepo @Inject constructor(
    ...
    fun submitResult(vararg results: TaskResult) {
        addResult(*results)
    ...
    fun addResult(vararg results: TaskResult): Single<List<Long>> = Observable.just(results)
    ...
    val results: Flowable<List<TaskResult>> = database.storedResultDao().getAll()
