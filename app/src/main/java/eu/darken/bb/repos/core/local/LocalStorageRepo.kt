...
import eu.darken.bb.repos.core.*
...
class LocalStorageRepo(
        ...
        private val repoRef: LocalStorageRepoRef
) : BackupRepo {
    ...
    override fun status(): Observable<RepoInfo> {
        return Observable.just(RepoInfo(
                id = repoRef.repoId,
    ...
    override fun save(backup: Backup): BackupReference {
        val backupDir = File(dataDir, backup.config.backupName).tryMkDirs()
